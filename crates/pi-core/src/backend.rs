...
use pi_render::segmenter::{segment_markdown, Segment};
pub use pi_render::RowSpec;
use pi_render::{
    first_line, format_tokens, hydrate_rowspecs, spec_for_segment, tail, tool_summary,
    user_content_text, TOOL_DETAIL_LIMIT,
};
use pi_rpc::{
...
use crate::palette;
use local::panel::{
    collect_rapid_mlx_snapshot, demo_rapid_mlx_snapshot, fetch_router_state, format_auth_entries,
    format_hf_results, format_ollama_panel, format_rapid_mlx_panel, format_router_models,
...
// ---------------------------------------------------------------------------
// RowSpec (plain, UI-toolkit-agnostic row descriptions) lives in `pi-render`
// now, re-exported above — shared with `hydrate_rowspecs`'s history-hydration
// path (also `pi-render`) and, across FFI, `pi-core-ffi`'s `RowRecord`. Each
// frontend converts `RowSpec` to its own native row/view type (Slint:
// `row_to_slint` in slinty-pi; Swift: `RowRecord` conversions).
// ---------------------------------------------------------------------------
